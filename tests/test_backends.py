...
def test_p5netcdf_zarr3(data_dir):
    ...
    with p5netcdf.Dataset(dataset) as p:
        ...
        assert p.dump(display=False) == f"""{p.dataset_name}: <p5netcdf.Dataset: /, 0 dimensions, 1 variable, 1 group>
    Attributes:
        Conventions: 'CF-1.13'
        global_attr_1: 3.14
        global_attr_2: 'foo'
...
def test_p5netcdf_zarr2(data_dir):
    ...
    with p5netcdf.Dataset(dataset) as p:
        assert p.backend == "zarr"
        assert p.dump(display=False) == f"""{p.dataset_name}: <p5netcdf.Dataset: /, 3 dimensions, 6 variables, 0 groups>
    Attributes:
        Conventions: 'CF-1.12'
    Dimensions:
...
def test_p5netcdf_ppfive(data_dir):
    ...
    with p5netcdf.Dataset(dataset, backend='ppfive') as p:
        ...
        assert p.dump(display=False) == f"""{p.dataset_name}: <p5netcdf.Dataset: /, 5 dimensions, 9 variables, 0 groups>
    Attributes:
        Conventions: 'CF-1.14'
    Dimensions:
...
def test_p5netcdf_h5py(data_dir):
    ...
    with p5netcdf.Dataset(dataset, backend='h5py') as p:
        ...
        assert p.dump(display=False) == f"""{p.dataset_name}: <p5netcdf.Dataset: /, 1 dimension, 1 variable, 1 group>
    Attributes:
        Conventions: 'CF-1.13'
        global_attr_1: np.float64(3.14)
        global_attr_2: 'foo'
