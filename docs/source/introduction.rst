...
`xnetcdf.Variable` objects have a `~xnetcdf.Variable.chunks` attribute
and a `~xnetcdf.Variable.chunking` method -- the former is also found
in the `pyfive` and `h5netcdf` libraries, the latter in the `netCDF4`
library.
